...
public class DanhSachVeMayBay implements IQuanLy<VeMayBay>, IFileHandler, IThongKe {
  ...
  private VeMayBay taoVeTuDataXML(Map<String, String> data) {
    try {
      ...
      // Xác định loại vé dựa trên các trường có sẵn
      if (data.containsKey("LoaiDoUong")) {
        // VeThuongGia
        return new VeThuongGia(
            ...
            XMLUtils.stringToBoolean(data.get("PhongChoVIP")), XMLUtils.stringToDouble(data.get("SoKgHanhLyKyGui")),
            data.get("LoaiDoUong"));
      } else if (data.containsKey("DoAn")) {
        // VePhoThong
  ...
  public boolean ghiFile(String tenFile) {
    try {
      ...
      for (VeMayBay ve : danhSach) {
        ...
        data.put("MaKhachHang", ve.getmaKH());
  ...
  public Map<String, Object> thongKeTongHop(Date from, Date to) {
    ...
    double doanhThuTrungBinh = (int) thongKeCoBan.get("tongVe") > 0
        ? (double) thongKeCoBan.get("tongDoanhThu") / (int) thongKeCoBan.get("tongVe")
        : 0;
