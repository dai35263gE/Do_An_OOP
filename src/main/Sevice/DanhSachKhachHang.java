...
public class DanhSachKhachHang implements IQuanLy<KhachHang>, IFileHandler {
    ...
    return danhSach.stream()
        .collect(Collectors.groupingBy(
            kh -> {
              ...
              if (diaChi.contains("hà nội"))
                return "Hà Nội";
              else if (diaChi.contains("tp.hcm") || diaChi.contains("hồ chí minh"))
                return "TP.HCM";
              else if (diaChi.contains("đà nẵng"))
                return "Đà Nẵng";
              else if (diaChi.contains("cần thơ"))
                return "Cần Thơ";
              else if (diaChi.contains("hải phòng"))
                return "Hải Phòng";
              else
                return "Tỉnh khác";
            },
  ...
  public void hienThiThongKe() {
    ...
    thongKeHang.forEach((hang, soLuong) -> System.out.printf("   - %s: %d khách hàng%n", hang, soLuong));

    Map<String, Integer> thongKeGioiTinh = thongKeTheoGioiTinh();
    System.out.println("Phân bố giới tính:");
    thongKeGioiTinh.forEach((gioiTinh, soLuong) -> System.out.printf("   - %s: %d khách hàng%n", gioiTinh, soLuong));
