...
public class KhachHang extends NguoiDung {
    ...
    public boolean huyVe(VeMayBay ve) {
        ...
        // SỬA LỖI: Kiểm tra vé trong hóa đơn (lichSuHoaDon), không dùng list veDaDat
        boolean timThayVe = false;
        for (HoaDon hd : this.lichSuHoaDon) {
            if (hd.getDanhSachVe().stream().anyMatch(v -> v.getMaVe().equals(ve.getMaVe()))) {
                timThayVe = true;
                break;
            }
        }
        ...
        try {
            // Sử dụng hàm getThongBaoKhongTheHuy() (hàm này có logic đúng)
            String thongBao = ve.getThongBaoKhongTheHuy(); //
            if (thongBao != null) { // Nếu có thông báo, tức là KHÔNG thể hủy
                throw new IllegalStateException(thongBao);
            }

            // Nếu không có thông báo (null), tức là CÓ THỂ HỦY
            ve.setTrangThai(VeMayBay.TRANG_THAI_DA_HUY);
            return true;
        } catch (IllegalStateException e) {
            System.err.println("Lỗi KhachHang.huyVe: " + e.getMessage());
            return false;
        }
    }
    ...
    public String kiemTraKhaNangHuyVe(VeMayBay ve) {
        ...
        // Giữ logic kiểm tra của VeMayBay (hàm này đã đúng)
