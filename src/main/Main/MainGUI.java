...
private void moDialogSuaChuyenBay() {
    ...
    JSpinner spinnerSoGheTrong = new JSpinner(new SpinnerNumberModel(cbCanSua.getSoGheTrong(), 0, cbCanSua.getSoGhe(), 1));
    JTextField txtMaMayBay = new JTextField(cbCanSua.getMaMayBay());
    JSpinner spinnerGiaCoBan = new JSpinner(new SpinnerNumberModel(cbCanSua.getGiaCoBan(), 500000.0, 50000000.0, 100000.0));
...
private void timKiemChuyenBay() {
    ...
    btnTimKiem.addActionListener(e -> {
        try {
            ...
            switch (loaiTim) {
                ...
                case "Theo mã chuyến bay":
                    ...
                    if (!maChuyen.isEmpty()) {
                        ...
                        JOptionPane.showMessageDialog(dialog, "Vui lòng nhập mã chuyến bay!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                    break;
                ...
                case "Tìm kiếm gần đúng":
                    ...
                    if (!keyword.isEmpty()) {
                        ...
                        JOptionPane.showMessageDialog(dialog, "Vui lòng nhập từ khóa tìm kiếm!", "Lỗi", JOptionPane.ERROR_MESSAGE);
                        return;
                    }
                    break;
            ...
            if (ketQua.isEmpty()) {
                JOptionPane.showMessageDialog(dialog, "Không tìm thấy chuyến bay nào phù hợp!", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
            }
    ...
    btnChon.addActionListener(e -> {
        ...
        if (selectedRow == -1) {
            JOptionPane.showMessageDialog(dialog, "Vui lòng chọn một chuyến bay!", "Thông báo", JOptionPane.WARNING_MESSAGE);
            return;
        }
        
        // Có thể thêm logic xử lý khi chọn chuyến bay ở đây
        String maChuyen = (String) tableKetQua.getValueAt(selectedRow, 0);
        JOptionPane.showMessageDialog(dialog, "Đã chọn chuyến bay: " + maChuyen, "Thông báo", JOptionPane.INFORMATION_MESSAGE);
    });
...
private void locChuyenBay() {
    ...
    // Lọc theo điểm đi
    String[] diaDiem = {"-- Tất cả điểm đi --", "Hà Nội (HAN)", "TP.HCM (SGN)", "Đà Nẵng (DAD)", "Nha Trang (CXR)", "Phú Quốc (PQC)", "Huế (HUI)"};
    JComboBox<String> cbDiemDi = new JComboBox<>(diaDiem);
    ...
    DocumentListener docListener = new DocumentListener() {
        public void insertUpdate(DocumentEvent e) { updateThongTin.run(); }
        public void removeUpdate(DocumentEvent e) { updateThongTin.run(); }
        public void changedUpdate(DocumentEvent e) { updateThongTin.run(); }
    };
...
private JPanel taoThongTinChungPanel(ChuyenBay cb) {
    ...
    addInfoRow2(panel, gbc, 6, "📏 Khoảng Cách:", String.format("%.0f km", ChuyenBay.tinhKhoangCach(cb.getDiemDi(), cb.getDiemDen())));
...
private JPanel taoThongTinThongKePanel(ChuyenBay cb) {
    ...
    addInfoRow(panel, gbc, 1, "📈 Ghế Đã Đặt:", soGheDaDat + "/" + cb.getSoGhe() + " (" + String.format("%.1f", tyLeDat) + "%)");
    addInfoRow(panel, gbc, 2, "📈 Ghế Trống:", cb.getSoGheTrong() + "/" + cb.getSoGhe() + " (" + String.format("%.1f", 100-tyLeDat) + "%)");
...
private String getTrangThaiMoTa(String trangThai) {
    switch (trangThai) {
        case ChuyenBay.TRANG_THAI_CHUA_BAY: return "Chuyến bay chưa khởi hành, có thể đặt vé";
        case ChuyenBay.TRANG_THAI_DANG_BAY: return "Chuyến bay đang trong hành trình";
        case ChuyenBay.TRANG_THAI_DA_BAY: return "Chuyến bay đã hoàn thành";
        case ChuyenBay.TRANG_THAI_HUY: return "Chuyến bay đã bị hủy, không thể đặt vé";
        default: return "";
    }
}
...
private String getLoaiMayBay(String maMayBay) {
    switch (maMayBay) {
        case "VN-A321": return "Airbus A321 (180-220 chỗ)";
        case "VN-B787": return "Boeing 787 Dreamliner (250-290 chỗ)";
        case "VN-A350": return "Airbus A350 (300-350 chỗ)";
