...
public class ThongKeDialogs {
    ...
    private JPanel taoTabDoanhThuTheoThang() {
        ...
        // Tạo bảng hiển thị
        String[] columns = {"Tháng/Năm", "Doanh Thu (VND)", "Tỷ Lệ (%)"};
        DefaultTableModel model = new DefaultTableModel(columns, 0);

        double tongDoanhThu = doanhThuTheoThang.values().stream().mapToDouble(Double::doubleValue).sum();

        doanhThuTheoThang.entrySet().stream()
                .sorted(Map.Entry.<String, Double>comparingByValue().reversed())
                .forEach(entry -> {
                    double tyLe = (entry.getValue() / tongDoanhThu) * 100;
        ...
        // Panel tổng hợp
        JPanel panelTongHop = new JPanel(new GridLayout(2, 2, 10, 5));
        panelTongHop.setBorder(BorderFactory.createTitledBorder("Tổng hợp"));
        panelTongHop.add(new JLabel("Tổng doanh thu:"));
        panelTongHop.add(new JLabel(String.format("%,.0f VND", tongDoanhThu)));
        panelTongHop.add(new JLabel("Tháng cao nhất:"));
        panelTongHop.add(new JLabel(doanhThuTheoThang.entrySet().stream()
                .max(Map.Entry.comparingByValue())
                .map(entry -> entry.getKey() + " - " + String.format("%,.0f VND", entry.getValue()))
                .orElse("N/A")));
    ...
    private JPanel taoTabTuyenBayPhoBien() {
        ...
        // Tạo bảng hiển thị
        String[] columns = {"Tuyến Bay", "Số Vé Bán", "Tỷ Lệ (%)"};
        DefaultTableModel model = new DefaultTableModel(columns, 0);

        long tongSoVe = tuyenBayPhoBien.values().stream().mapToLong(Long::longValue).sum();

        tuyenBayPhoBien.entrySet().stream()
                .sorted(Map.Entry.<String, Long>comparingByValue().reversed())
                .forEach(entry -> {
                    double tyLe = (entry.getValue() * 100.0 / tongSoVe);
                    model.addRow(new Object[]{
                        entry.getKey(),
                        entry.getValue() + " vé",
                        String.format("%.1f%%", tyLe)
        ...
        panel.add(new JLabel("THỐNG KÊ TUYẾN BAY PHỔ BIẾN"), BorderLayout.NORTH);
        panel.add(scrollPane, BorderLayout.CENTER);
