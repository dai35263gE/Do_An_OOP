...
public class UsersGUI extends JFrame {
  ...
  private JButton btnXemHoaDon, btnHuyVe, btnXemChiTietVe, btnThanhToan;
  ...
  private JPanel createTabVeCuaToi() {
    ...
    // === SỬA ĐỔI: Thêm nút Thanh Toán ===
    btnThanhToan = new JButton("Thanh Toán Vé");
    btnThanhToan.setBackground(new Color(60, 179, 113)); // Màu xanh
    ...
    panelButton.add(btnThanhToan); // Thêm nút vào panel
  ...
  private void setupEvents() {
    ...
    // Tab Vé của tôi
    btnThanhToan.addActionListener(e -> thanhToanVe());
  ...
  private void datVe() {
    ...
    if (!kiemTraChuyenBayKhaDung(chuyenBay, soLuongDat)) return;
  ...
  private boolean xuLyDatVe(List<VeMayBay> danhSachVeMoi, ChuyenBay chuyenBay) {
    try {
      ...
      for (VeMayBay ve : danhSachVeMoi) {
        if (!dsVe.them(ve)) {
          JOptionPane.showMessageDialog(this, "Lỗi khi thêm vé " + ve.getMaVe() + " vào hệ thống!", "Lỗi", JOptionPane.ERROR_MESSAGE);
          // Cần logic rollback phức tạp ở đây, tạm thời bỏ qua
          return false;
        }
        if (!chuyenBay.themVe(ve)) { // Đã bao gồm cả datGhe()
          JOptionPane.showMessageDialog(this, "Lỗi khi thêm vé " + ve.getMaVe() + " vào chuyến bay!", "Lỗi", JOptionPane.ERROR_MESSAGE);
  ...
  private boolean taoHoaDon(List<VeMayBay> danhSachVeMoi, double tongGiamGia) {
    try {
      ...
      HoaDon hoaDon = new HoaDon(maHoaDon, khachHangDangNhap, danhSachVeMoi, tongGiamGia, HoaDon.PT_CHUYEN_KHOAN);
  ...
  private void capNhatDiemTichLuy(double tongGiaVeSauGiam) {
    try {
      int diemThuong = (int) (tongGiaVeSauGiam / 100000); // 1 điểm cho mỗi 100,000 VND
  ...
  private void hienThiThongBaoThanhCong(List<VeMayBay> danhSachVeMoi, ChuyenBay chuyenBay) {
    ...
    int diemThuong = (int) (tongGia / 100000);
  ...
  private boolean hienThiThongTinVeXacNhan(List<VeMayBay> danhSachVe, ChuyenBay chuyenBay, double tongGiamGia, double tongThanhTien) {
  ...
  private VeMayBay hienThiDialogDatVe(ChuyenBay chuyenBay, String dialogTitle, List<String> gheDaChonTruoc) {
    ...
    btnDatVe.addActionListener(e -> {
      ...
      switch (loaiVe) {
        case "THƯƠNG GIA":
          String maVe = "VG" + String.format("%03d", quanLy.getDsVe().demSoLuong() + (int)(Math.random() * 1000));
        ...
        case "PHỔ THÔNG":
          String maVe1 = "VP" + String.format("%03d", quanLy.getDsVe().demSoLuong() + (int)(Math.random() * 1000));
        ...
        case "TIẾT KIỆM":
          String maVe2 = "VT" + String.format("%03d", quanLy.getDsVe().demSoLuong() + (int)(Math.random() * 1000));
  ...
  private void hienThiVeTrongBang(List<VeMayBay> danhSachVe) {
    ...
    // Thông báo kết quả
    if (count == 0 && (danhSachVe != null && danhSachVe.size() > 0) ) { // Sửa: Chỉ thông báo nếu có vé nhưng bị ẩn (vé hủy)
      JOptionPane.showMessageDialog(this,
  ...
  public static void showDangKy(QuanLyBanVeMayBay quanLy) {
    ...
    gbc.gridx = 1; panel.add(txtHoTen, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("CMND/CCCD:*"), gbc);
    gbc.gridx = 1; panel.add(txtCmnd, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Số điện thoại:*"), gbc);
    gbc.gridx = 1; panel.add(txtSoDT, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Email:*"), gbc);
    gbc.gridx = 1; panel.add(txtEmail, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Ngày sinh (dd/MM/yyyy):*"), gbc);
    gbc.gridx = 1; panel.add(txtNgaySinh, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Giới tính:*"), gbc);
    gbc.gridx = 1; panel.add(cboGioiTinh, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Địa chỉ:*"), gbc);
    gbc.gridx = 1; panel.add(txtDiaChi, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Mật khẩu:*"), gbc);
    gbc.gridx = 1; panel.add(txtMatKhau, gbc); gbc.gridx = 0; gbc.gridy++;
    panel.add(new JLabel("Nhập lại mật khẩu:*"), gbc);
    gbc.gridx = 1; panel.add(txtNhapLaiMatKhau, gbc); gbc.gridx = 0; gbc.gridy++;
  ...
  private void thanhToanVe() {
    int row = tableVeCuaToi.getSelectedRow();
    if (row < 0) {
      JOptionPane.showMessageDialog(this, "Vui lòng chọn một vé để thanh toán!", "Thông báo", JOptionPane.WARNING_MESSAGE);
      return;
    }

    String maVe = (String) modelVeCuaToi.getValueAt(row, 0);
    String trangThaiVe = (String) modelVeCuaToi.getValueAt(row, 6);

    // Kiểm tra trạng thái hiển thị trên GUI trước
    if (trangThaiVe.equals("Đã thanh toán")) {
      JOptionPane.showMessageDialog(this, "Vé này đã được thanh toán trước đó.", "Thông báo", JOptionPane.INFORMATION_MESSAGE);
      return;
    }
    if (trangThaiVe.equals("Đã hủy")) {
      JOptionPane.showMessageDialog(this, "Vé này đã bị hủy, không thể thanh toán.", "Thông báo", JOptionPane.ERROR_MESSAGE);
      return;
    }

    // Tìm hóa đơn chứa vé này
    HoaDon hoaDonCuaVe = null;
    for (HoaDon hd : khachHangDangNhap.getLichSuHoaDon()) {
      ...
      if (hoaDonCuaVe != null) break;
    }

    if (hoaDonCuaVe == null) {
      JOptionPane.showMessageDialog(this, "Lỗi: Không tìm thấy hóa đơn cho vé này.", "Lỗi hệ thống", JOptionPane.ERROR_MESSAGE);
      return;
    }

    // Xác nhận thanh toán
    String thongBao = String.format(
            "Xác nhận thanh toán cho hóa đơn %s?\n\n" +
                    "Tổng tiền: %,.0f VND\n" +
                    "Trạng thái: %s",
            hoaDonCuaVe.getMaHoaDon(),
            hoaDonCuaVe.getThanhTien(),
            chuyenTrangThaiSangText1(hoaDonCuaVe.getTrangThai()) // Sửa: Dùng hàm chuyển đổi
    );

    int confirm = JOptionPane.showConfirmDialog(this, thongBao, "Xác nhận thanh toán", JOptionPane.YES_NO_OPTION);

    if (confirm == JOptionPane.YES_OPTION) {
      try {
        // Thực hiện thanh toán
        hoaDonCuaVe.thanhToan();
        ...
        // Tải lại dữ liệu trên GUI
        taiVeCuaToi();
        taiLichSu();

      } catch (IllegalStateException ex) {
        // Bắt lỗi nếu hóa đơn đã được thanh toán hoặc đã hủy
        ValidatorUtils.showErrorDialog(this, "Không thể thanh toán: " + ex.getMessage());
      } catch (Exception ex) {
        ValidatorUtils.showExceptionDialog(this, "Lỗi trong quá trình thanh toán:", ex);
      }
    }
  }
  ...
  private String moDialogChonGhe(ChuyenBay chuyenBay, List<String> gheDaChonTruoc) {
    ...
    // Lấy danh sách ghế đã đặt từ logic của bạn (Đã sửa lỗi liên kết dữ liệu)
